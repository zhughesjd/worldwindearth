...
public class KMLViewer extends ApplicationTemplate
{
	public static class AppFrame extends ApplicationTemplate.AppFrame
	{
		...
		public AppFrame()
		{
			...
			dialog.setContentPane(new JScrollPane(new JTree(model)));
	...
	public static class WorkerThread extends Thread
	{
		...
		public void run()
		{
			try
			{
				...
				KMLPlacemark pm = createPlacemark(finalKMLRoot.getNamespaceURI( ),-95,39);
				appFrame.model.add( pm );
				try {
					Thread.sleep(5000l);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				pm.applyChange( createPlacemark(finalKMLRoot.getNamespaceURI( ),-96,40));
			}
			catch (Exception e)
			{
				e.printStackTrace();
